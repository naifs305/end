...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setPageLoading(true);
    const endpoint = (isAdmin || isSupervisor) ? '/analytics/manager' : '/analytics/employee';
    api
      .get(endpoint)
  ...
  const quickCards = useMemo(() => {
    ...
    if (isAdmin || isSupervisor) {
      return [
        ...
        {
          title: isSupervisor ? 'موظفو المشروع' : 'المستخدمون المسجلون',
          value: dashboard.totalUsers || 0,
          href: isSupervisor ? '/kpis' : '/users',
          color: 'primary',
        },
      ];
    }
  ...
  }, [dashboard, isAdmin, isSupervisor]);
      ...
      <div className="min-h-full bg-background p-4 md:p-6">
        <div className="mb-6 rounded-3xl border border-border bg-white p-6 shadow-card">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-xl font-extrabold text-text-main md:text-2xl">
                مرحبًا، {user.firstName}
              </h1>
              <p className="mt-2 text-sm text-text-soft">
                لوحة متابعة سريعة مختصرة حسب الدور النشط
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              {canCreateCourse(activeRole) && (
                <Link
                  href="/courses/create"
                  className="inline-flex items-center rounded-2xl bg-primary px-4 py-2 text-sm font-extrabold text-white hover:bg-primary-dark"
                >
                  إضافة دورة جديدة
                </Link>
              )}
        ...
        {isAdmin || isSupervisor ? (
          <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3">
            <InfoPanel title={isSupervisor ? 'ملخص فريق المشروع' : 'ملخص المستخدمين'}>
              <InfoRow label="الموظفون" value={dashboard.employeesCount || 0} />
              <InfoRow label="مشرفو المشاريع" value={dashboard.supervisorsCount || 0} />
              {!isSupervisor && <InfoRow label="المديرون" value={dashboard.managersCount || 0} />}
              {!isSupervisor && <InfoRow label="مستخدمو الجودة" value={dashboard.qualityViewersCount || 0} />}
            </InfoPanel>
            ...
            <InfoPanel title="ملخص الأداء الشهري">
              ...
              <InfoRow
                ...
                value={
                  dashboard.topPerformer
                    ? `${dashboard.topPerformer.user?.firstName || ''} ${dashboard.topPerformer.user?.lastName || ''}`.trim() || '-'
                    : '-'
                }
                compact
              />
              <InfoRow
                label="الأقل أداءً"
                value={
                  dashboard.lowPerformer
                    ? `${dashboard.lowPerformer.user?.firstName || ''} ${dashboard.lowPerformer.user?.lastName || ''}`.trim() || '-'
                    : '-'
                }
                compact
              />
              ...
              {dashboard.latestCourses.map((course) => (
                <Link key={course.id} href={`/courses/${course.id}`}>
                  <div className="cursor-pointer rounded-2xl border border-border bg-background px-4 py-3 transition hover:border-primary/30 hover:bg-primary-light/30">
                    <div className="mb-1 line-clamp-2 text-sm font-extrabold text-text-main">
                      {course.name || '-'}
                    </div>
                    <div className="mb-1 text-xs text-text-soft">
