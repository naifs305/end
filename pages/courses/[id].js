...
export default function CourseDetail() {
  ...
  return (
    <MainLayout breadcrumb={[{ label: t('nav.courses'), href: '/courses' }, { label: course.name }]}>
      ...
      {showNotesReport && <CourseNotesReportForm courseId={id} course={course} onClose={() => setShowNotesReport(false)} />}

      <ReasonModal
        open={!!overrideModal}
        title={overrideModal?.action === 'revert' ? t('courseDetail.revert') : t('courseDetail.exempt')}
        label={overrideModal?.action === 'revert' ? t('courseDetail.revertPrompt') : t('courseDetail.exemptPrompt')}
        required
        tone={overrideModal?.action === 'revert' ? 'danger' : 'warning'}
