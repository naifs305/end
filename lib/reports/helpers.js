...
function renderComment(text) {
  if (!text) return '<span style="color:#6b7280;font-style:italic;">لا توجد ملاحظات مسجلة في هذا المحور</span>';
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  const isList = lines.some(l => /^[\*\-•]/.test(l) || /^[\d١٢٣٤٥٦٧٨٩٠]+[.)]\s/.test(l));
  if (isList && lines.length > 1) {
    const items = lines.map(l => l.replace(/^[\*\-•]\s*/, '').replace(/^[\d١٢٣٤٥٦٧٨٩٠]+[.)]\s*/, '').trim()).filter(Boolean);
    return `<ul style="margin:0; padding-right:18px; font-size:12px; line-height:1.75; color:#334155;">${items.map(i => `<li style="margin-bottom:3px;">${escapeHtml(i)}</li>`).join('')}</ul>`;
  }
  return `<span style="font-size:12px; line-height:1.7; color:#334155;">${escapeHtml(text)}</span>`;
}
...
function renderTitleBox(courseName, tagline) {
  return `
    <div style="margin:10px 0 12px; padding:12px 14px; background:linear-gradient(135deg,#016564 0%,#498983 100%); border-radius:14px;">
      <h2 style="margin:0; color:#ffffff; font-size:19px; font-weight:900; line-height:1.35; font-family:'Cairo',Tahoma,Arial,sans-serif;">${escapeHtml(courseName)}</h2>
      <p style="margin:6px 0 0; color:#ffffff; font-size:12px; opacity:0.95; font-family:'Cairo',Tahoma,Arial,sans-serif;">${escapeHtml(tagline)}</p>
    </div>
  `;
}
