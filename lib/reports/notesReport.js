...
const CATEGORY_LABELS = {
  ...
  trainees: 'تتعلق بالمتدربين',
  trainer: 'تتعلق بالمدرب',
};
...
function renderNotesReport({ course, data = {} }, options = {}) {
  ...
  const beneficiaryEntity = data.beneficiaryEntity || course.beneficiaryEntity || '-';
  const attendanceCount = data.attendanceCount != null && data.attendanceCount !== ''
    ? String(data.attendanceCount)
    : '-';
  const executingPartner = data.executingPartner || '-';
  const additionalTrainers = data.additionalTrainers || '-';
  const category = CATEGORY_LABELS[data.category] || '-';
  const priority = PRIORITY_LABELS[data.priority] || '-';
...
<body>
  <div class="page">
    ...
    <div class="section">
      ...
      <div class="section-body">
        <div class="info-grid">
          ...
          <div class="card">
            <div class="label">الجهة المستفيدة</div>
            <div class="value">${escapeHtml(beneficiaryEntity)}</div>
          </div>
          <div class="card">
            <div class="label">عدد الحضور</div>
            <div class="value">${escapeHtml(attendanceCount)}</div>
          </div>
          <div class="card">
            <div class="label">الجهة المنفذة / شريك التنفيذ</div>
            <div class="value">${escapeHtml(executingPartner)}</div>
          </div>
          <div class="card">
            <div class="label">مدربون إضافيون / خارجيون</div>
            <div class="value">${escapeHtml(additionalTrainers)}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="section">
      <div class="section-header">تصنيف الملاحظة والإجراء المقترح</div>
      <div class="section-body">
        <div class="info-grid">
          <div class="card">
            <div class="label">تصنيف الملاحظة</div>
            <div class="value">${escapeHtml(category)}</div>
          </div>
          <div class="card">
            <div class="label">مستوى الأولوية</div>
            <div class="value">${escapeHtml(priority)}</div>
          </div>
        </div>
        <div class="note-box" style="margin-top:10px;">
          <strong>الإجراء المقترح / الجهة المقترح تحويل الملاحظة لها:</strong><br/>
