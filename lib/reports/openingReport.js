...
function renderOpeningReport({ course, element, data }) {
  ...
  const registeredCount =
    data.registered_trainees_count ?? info.traineesCount ?? course.numTrainees ?? '-';
  ...
  const initialAttendanceRate =
    ...
    typeof initialAttendance === 'number' &&
    registeredCount > 0
      ? `${((initialAttendance / registeredCount) * 100).toFixed(1)}%`
      : '-';
  ...
  const evaluationSections = [
    ...
    {
      title: 'تقييم الجاهزية التشغيلية',
      rating: data.initial_readiness?.rating,
      comment: data.initial_readiness?.comment,
    },
    {
      title: 'تقييم حضور والتزام المتدربين',
      rating: data.trainee_attendance?.rating,
      comment: data.trainee_attendance?.comment,
    },
  ];
