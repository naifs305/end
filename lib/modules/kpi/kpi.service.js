...
function buildViewModel(base) {
  const isSubjectToEvaluation = !(
    base.assignedCoursesCount === 0 && base.actualCoursesCount === 0 && !base.hasExecutedElements
  );
...
function generateInsights(metrics, scores, elementBreakdown = []) {
  ...
  // رؤية تقدير التطوّع
  if (scores.voluntaryReportBonus > 0) {
    insights.push({ type: 'positive', text: `قدّم تقارير افتتاح/اختتام تطوّعية غير إجبارية على دوراته واعتُمدت — تقدير لجهد إضافي (+${scores.voluntaryReportBonus} نقطة).` });
  }
...
function computeEmployeeMetrics(relevantCourses, relevantElements) {
  ...
  // إصلاح: ساعات الاسترداد بعد الإرجاع — العناصر التي executionAt > decisionAt تعني أن الموظف أعاد التقديم بعد قرار الإرجاع
  const resubmissionHours = relevantElements
    .filter((el) => el.decisionAt && el.executionAt &&
      new Date(el.executionAt) > new Date(el.decisionAt))
    .map((el) => (new Date(el.executionAt).getTime() - new Date(el.decisionAt).getTime()) / 3600000);
  ...
  // ── المؤشر الجديد ١: العناصر المقدَّمة قبل الموعد المثالي (Proactive Rate) ──
  const proactiveElements = submittedElements.filter((el) => {
    const course = relevantCourses.find((c) => c.id === el.courseId);
    if (!course || !el.element?.deadlineRefPoint || el.element?.deadlineIdealHours == null) return false;
    const MS = 3600000;
    const refDate = el.element.deadlineRefPoint === 'START' ? new Date(course.startDate) : new Date(course.endDate);
    const idealDeadline = new Date(refDate.getTime() + el.element.deadlineIdealHours * MS);
    return new Date(el.executionAt) <= idealDeadline;
  });

  // ── المؤشر الجديد ٢: العناصر التي طُلب لها تمديد ──
  const elementsWithExtension = relevantElements.filter((el) => (el.extensionHours || 0) > 0);

  // ── المؤشر الجديد ٣: الدورات بلا أخطاء (Zero-Defect Courses) ──
  // دورة بلا أخطاء = جميع عناصرها مقدَّمة ومعتمدة بدون أي إرجاع/رفض وبدون إعادة تقديم
  const elementsByCourse = new Map();
  for (const el of relevantElements) {
    if (el.status === 'NOT_APPLICABLE') continue;
    const arr = elementsByCourse.get(el.courseId) || [];
    arr.push(el);
    elementsByCourse.set(el.courseId, arr);
  }
  const coursesWithSubmissions = relevantCourses.filter((c) => {
    const els = elementsByCourse.get(c.id) || [];
    return els.some((el) => el.status !== 'NOT_STARTED');
  });
  const zeroDefectCourses = coursesWithSubmissions.filter((course) => {
    const els = elementsByCourse.get(course.id) || [];
    if (!els.length) return false;
    return els.every((el) => {
      if (['RETURNED', 'REJECTED', 'NOT_STARTED'].includes(el.status)) return false;
      // كشف إعادة التقديم: إذا executionAt > decisionAt يعني الموظف أعاد التقديم بعد الإرجاع
      if (el.decisionAt && el.executionAt && new Date(el.executionAt) > new Date(el.decisionAt)) return false;
      return true;
    });
  });
...
async function calculateAndStore(periodType, year, value, managerId, startOverride, endOverride, labelOverride) {
  ...
  // منشئ الدورة هو المسؤول عن الدورة نفسها (إسناد/تغطية الدورات)
  const coursesByOwner = new Map();
  for (const course of courses) {
    if (!employeeIds.includes(course.primaryEmployeeId)) continue;
    const arr = coursesByOwner.get(course.primaryEmployeeId) || [];
    arr.push(course);
    coursesByOwner.set(course.primaryEmployeeId, arr);
  }

  // كل عنصر إقفال يُحسب لمن نفّذه فعلياً (وليس بالضرورة لمالك الدورة)
  const executedElementsByUser = new Map();
  for (const course of courses) {
    for (const el of course.closureElements || []) {
      if (el.status === 'NOT_APPLICABLE') continue;
      if (!el.executedById) continue;
      if (el.executedById === course.primaryEmployeeId) continue; // محسوب أصلاً ضمن دورات مالكها
      if (!employeeIds.includes(el.executedById)) continue;
      const arr = executedElementsByUser.get(el.executedById) || [];
      arr.push({ el, course });
      executedElementsByUser.set(el.executedById, arr);
    }
  }
  ...
  for (const employee of employees) {
    ...
    const ownedCourses = coursesByOwner.get(employee.id) || [];
    // عناصر دوراته التي لم ينفّذها موظف آخر (إن نفّذها غيره تُحسب لذلك الغير)
    const ownedElements = ownedCourses
      .flatMap((c) => (c.closureElements || []).filter((el) => el.status !== 'NOT_APPLICABLE'))
      .filter((el) => !el.executedById || el.executedById === employee.id);

    // عناصر نفّذها هذا الموظف فعلياً على دورات لا يملكها
    const executedElsewhere = executedElementsByUser.get(employee.id) || [];
    const extraElements = executedElsewhere.map((x) => x.el);
    const ownedCourseIds = new Set(ownedCourses.map((c) => c.id));
    const extraCourses = [];
    const seenExtraCourseIds = new Set();
    for (const { course } of executedElsewhere) {
      if (ownedCourseIds.has(course.id) || seenExtraCourseIds.has(course.id)) continue;
      seenExtraCourseIds.add(course.id);
      extraCourses.push(course);
    }

    const relevantCourses = [...ownedCourses, ...extraCourses];
    const relevantElements = [...ownedElements, ...extraElements];
    ...
    // مكافأة الدورات العابرة للمشاريع — تخص الدورات التي يملكها الموظف (منشئ الدورة)
    const crossProjectCourses = ownedCourses.filter(c => c.isCrossProject);
    ...
    // عدد الدورات الفعلي للموظف = الدورات التي هو مسؤولها (منشئها) — لا الدورات التي ساعد فيها فقط
    const actualCoursesCount = ownedCourses.length;
    ...
    // خاضع للتقييم إن كان مسؤولاً عن دورة، أو نفّذ عناصر إقفال فعلية بنفسه
    const isSubjectToEvaluation = actualCoursesCount > 0 || relevantElements.length > 0;
    ...
    const metrics = {
      ...
      // ── المؤشرات الأربعة الجديدة ──
      proactiveElementsCount: computed.proactiveElements.length,
      proactiveRate: toPercent(
        computed.proactiveElements.length,
        computed.submittedElements.filter(el => {
          const c = relevantCourses.find(x => x.id === el.courseId);
          return c && el.element?.deadlineRefPoint && el.element?.deadlineIdealHours != null;
        }).length,
      ),
      extensionRequestsCount: computed.elementsWithExtension.length,
      extensionRate: toPercent(computed.elementsWithExtension.length, relevantElements.length),
      zeroDefectCoursesCount: computed.zeroDefectCourses.length,
      zeroDefectCoursesRate: toPercent(computed.zeroDefectCourses.length, computed.coursesWithSubmissions.length),
      avgReturnRecoveryHours: toAverage(computed.resubmissionHours),
    };
    ...
    // ── مكافأة تقارير الافتتاح/الاختتام التطوّعية (دورات داخلية غير إجبارية) ──
    // المنطق: الموظف فعّلها بنفسه رغم أنها غير مطلوبة منه، وأكملها واعتُمدت — تقدير لجهد إضافي
    const voluntaryReportElements = relevantElements.filter((el) => {
      if (el.status !== 'APPROVED') return false;
      const key = el.element?.key;
      if (key !== 'opening_report' && key !== 'closing_report') return false;
      const course = relevantCourses.find((c) => c.id === el.courseId);
      if (!course) return false;
      return key === 'opening_report' ? course.openingReportVoluntary : course.closingReportVoluntary;
    });
    const voluntaryReportBonus = Math.min(6, voluntaryReportElements.length * 2); // +2 لكل تقرير، حد أقصى +6
    ...
    const scores = isSubjectToEvaluation ? {
      ...
      voluntaryReportBonus,
      reportBonusScore,
      // الدرجة النهائية = الأوزان الستة + مكافأة التقارير + مكافأة الدورات العابرة + مكافأة التقارير التطوّعية
      finalScore: clampScore(baseScore + reportBonusScore),
    } : { ...rawScores, reportBonusScore: 0, voluntaryReportBonus: 0 };
    ...
    const viewModel = buildViewModel({
      ...
      extraCoursesCount,
      assignmentCoverageRate,
      hasExecutedElements: extraElements.length > 0,
      closureCompletionRate: metrics.closureCompletionRate,
      submissionRate: metrics.submissionRate,
      overdueElementsRate: metrics.overdueElementsRate,
      stalePendingElementsRate: metrics.stalePendingElementsRate,
    ...
    snapshots.push({
      ...
      // المؤشرات الأربعة الجديدة (محسوبة في الجلسة، لا تُخزَّن في DB)
      proactiveRate: metrics.proactiveRate,
      extensionRate: metrics.extensionRate,
      extensionRequestsCount: metrics.extensionRequestsCount,
      zeroDefectCoursesCount: metrics.zeroDefectCoursesCount,
      zeroDefectCoursesRate: metrics.zeroDefectCoursesRate,
      avgReturnRecoveryHours: metrics.avgReturnRecoveryHours,
      ...viewModel,
    });
  }
...
async function getSnapshots(periodType, periodLabel, context = {}) {
  ...
  const enriched = snapshots.map((snapshot, idx) => {
    ...
    const vm = buildViewModel({
      ...
      extraCoursesCount,
      assignmentCoverageRate,
      hasExecutedElements: (snapshot.requiredElementsCount || 0) > 0,
      closureCompletionRate: snapshot.closureCompletionRate,
...
async function getEmployeeSnapshotDetails(userId, periodType, periodLabel, context = {}) {
  ...
  const [assignment, ownedCourses, executedCourses] = await Promise.all([
    ...
    repo.findExecutedCoursesWithElements(userId, snapshot.periodStart, snapshot.periodEnd),
  ]);
  ...
  // تفاصيل حسب نوع العنصر — كل عنصر يُحسب لمن نفّذه فعلياً
  const ownedElements = ownedCourses
    .flatMap((c) => (c.closureElements || []).filter((el) => el.status !== 'NOT_APPLICABLE'))
    .filter((el) => !el.executedById || el.executedById === userId);
  const extraElements = executedCourses.flatMap((c) =>
    (c.closureElements || []).filter((el) => el.status !== 'NOT_APPLICABLE' && el.executedById === userId),
  );
  const courses = [...ownedCourses, ...executedCourses];
  const relevantElements = [...ownedElements, ...extraElements];
  const elementBreakdown = buildElementBreakdown(courses, relevantElements);

  // حساب المؤشرات الأربعة الجديدة للعرض التفصيلي
  const detailComputed = computeEmployeeMetrics(courses, relevantElements);
  const submittedWithDeadline = detailComputed.submittedElements.filter(el => {
    const c = courses.find(x => x.id === el.courseId);
    return c && el.element?.deadlineRefPoint && el.element?.deadlineIdealHours != null;
  });
  const advancedMetrics = {
    proactiveRate: toPercent(detailComputed.proactiveElements.length, submittedWithDeadline.length),
    proactiveElementsCount: detailComputed.proactiveElements.length,
    extensionRate: toPercent(detailComputed.elementsWithExtension.length, relevantElements.length),
    extensionRequestsCount: detailComputed.elementsWithExtension.length,
    zeroDefectCoursesCount: detailComputed.zeroDefectCourses.length,
    zeroDefectCoursesRate: toPercent(detailComputed.zeroDefectCourses.length, detailComputed.coursesWithSubmissions.length),
    avgReturnRecoveryHours: toAverage(detailComputed.resubmissionHours),
  };

  // الرؤى الذكية
  const metricsForInsights = {
    isSubjectToEvaluation: !(assignedCoursesCount === 0 && actualCoursesCount === 0 && relevantElements.length === 0),
