...
async function getPendingApprovalsQueue(user, activeRole) {
  ...
  if (activeRole === 'PROJECT_SUPERVISOR') {
    const supervisedIds = await permissions.getSupervisedProjectIds(user.id);
    courseFilter = { operationalProjectId: { in: supervisedIds } };
  } else if (activeRole === 'MANAGER') {
    courseFilter = {};
  } else {
    courseFilter = await getVisibleCourseWhere(user, activeRole);
  }
  ...
  const projectIds = [...new Set(items.map((item) => item.course?.operationalProjectId).filter(Boolean))];
  const supervisors = projectIds.length
    ? await prisma.projectSupervisor.findMany({
        where: { operationalProjectId: { in: projectIds } },
        select: {
          operationalProjectId: true,
  ...
  return items.map((item) => ({
    ...
    supervisorName: supervisorsMap.get(item.course?.operationalProjectId) || '-',
    elementKey: item.element?.key || '-',
    elementName: item.element?.name || '-',
    employeeName: `${item.executedBy?.firstName || ''} ${item.executedBy?.lastName || ''}`.trim() || item.executedBy?.email || '-',
