...
function buildSingleCourseRelationFilter(courseWhere) {
  if (!courseWhere || Object.keys(courseWhere).length === 0) return undefined;
  return { is: courseWhere };
}
...
async function getManagerDashboard(user, activeRole, projectIdFilter) {
  ...
  const courseRelationFilter = buildSingleCourseRelationFilter(courseWhere);
  ...
  const [
    ...
    prisma.courseClosureTracking.count({
      where: {
        ...
        ...(courseRelationFilter ? { course: courseRelationFilter } : {}),
      },
    }),
