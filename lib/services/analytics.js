...
async function getPendingApprovalsQueue(user, activeRole) {
  ...
  const items = await prisma.courseClosureTracking.findMany({
    ...
    include: {
      ...
      decider: { select: { id: true, firstName: true, lastName: true, email: true } },
    },
    orderBy: { executionAt: 'asc' },
  });

  return items.map((item) => {
    const submittedBy = `${item.executor?.firstName || ''} ${item.executor?.lastName || ''}`.trim() || item.executor?.email || '-';
    const approverName = activeRole === 'MANAGER'
      ? 'المدير'
      : `${user.firstName || ''} ${user.lastName || ''}`.trim() || user.email || 'مشرف المشروع';
    ...
    return {
      ...
      ownerName: `${item.course?.primaryEmployee?.firstName || ''} ${item.course?.primaryEmployee?.lastName || ''}`.trim() || item.course?.primaryEmployee?.email || '-',
      approverName,
      submittedAt: item.executionAt || null,
      note: item.notes || '',
    };
  });
}
