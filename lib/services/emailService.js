...
// ── عنصر مرفوض ──────────────────────────────────────────────────────────────
async function sendElementRejectedEmail({ employeeId, courseName, elementName, reason, courseId }) {
  const user = await getUser(employeeId);
  if (!user?.email || !isRealEmail(user.email)) return;
  return email.sendElementRejected({
    to: user.email,
    firstName: user.firstName,
    courseName, elementName, reason, courseId,
  });
...
async function sendChallengeAnnouncementToAll({ challengeTitle, targetDesc, periodLabel }) {
  ...
  await Promise.all(
    users.map((u) => {
      ...
      return email.sendChallengeAnnouncement({
