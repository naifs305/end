...
async function initializeClosureElements(course, tx) {
  const db = tx || prisma;
  const allElements = await db.closureElement.findMany({ orderBy: { name: 'asc' } });
  ...
  const data = allElements.map((el) => {
    ...
    if (el.key === 'pre_test' && !course.requiresPreTest) status = 'NOT_APPLICABLE';
    if (el.key === 'post_test' && !course.requiresPostTest) status = 'NOT_APPLICABLE';
    if (el.key === 'supervisor_compensation' && !course.requiresSupervisorCompensation) status = 'NOT_APPLICABLE';
    if (el.key === 'trainer_compensation' && !course.requiresTrainerCompensation) status = 'NOT_APPLICABLE';
    // التأمين الطبي: إجباري للدورات الخارجية فقط
    if (el.key === 'medical_insurance' && course.courseType !== 'external') status = 'NOT_APPLICABLE';
    // التقارير: مشروطة — خارجية = إجبارية، داخلية/بُعد = اختيارية
    if (el.key === 'opening_report' && !course.requiresOpeningReport) status = 'NOT_APPLICABLE';
    if (el.key === 'closing_report' && !course.requiresClosingReport) status = 'NOT_APPLICABLE';
...
async function refreshConditionalElements(courseId, data) {
  const keysToUpdate = [
    ...
    { key: 'pre_test', value: data.requiresPreTest },
    { key: 'post_test', value: data.requiresPostTest },
    { key: 'supervisor_compensation', value: data.requiresSupervisorCompensation },
    { key: 'trainer_compensation', value: data.requiresTrainerCompensation },
    // التأمين الطبي: تحديث تلقائي عند تغيير نوع الدورة
    ...(data.courseType !== undefined
      ? [{ key: 'medical_insurance', value: data.courseType === 'external' }]
      : []),
    // التقارير: مشروطة
    ...(data.requiresOpeningReport !== undefined
      ? [{ key: 'opening_report', value: data.requiresOpeningReport }]
      : []),
    ...(data.requiresClosingReport !== undefined
      ? [{ key: 'closing_report', value: data.requiresClosingReport }]
      : []),
  ];
