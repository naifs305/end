    ...
    if (el.executionAt) {
      ...
      if (course) {
        const hours = Math.max(
          0,
          (new Date(el.executionAt).getTime() - new Date(course.createdAt).getTime()) / (1000 * 60 * 60),
        );
...
async function calculateAndStore(periodType, year, value, managerId) {
  ...
  for (const employee of employees) {
    ...
    const scores = isSubjectToEvaluation ? {
      productivityScore: rawScores.productivityScore,                          // 25%
      timelinessScore:   clampScore(timingScore),                              // 20%
      qualityScore:      rawScores.qualityScore,                               // 20%
      criticalScore:     clampScore(criticalScore),                            // 20%
      responsivenessScore: rawScores.speedScore,                               // 10%
      disciplineScore:   rawScores.disciplineScore,                            // 5%
