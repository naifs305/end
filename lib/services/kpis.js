...
async function calculateAndStore(periodType, year, value, managerId) {
  ...
  for (const employee of employees) {
    ...
    const stalePendingElements = relevantElements.filter((el) => {
      if (el.status !== 'NOT_STARTED' && el.status !== 'RETURNED') return false;
      const course = relevantCourses.find((c) => c.id === el.courseId);
      const baseDate = el.status === 'RETURNED' && el.decisionAt ? new Date(el.decisionAt) : course?.createdAt;
    ...
    const metrics = {
      ...
      operationalErrorRate: toPercent(returnedElements.length + rejectedElements.length, submittedElements.length),
...
async function getSnapshots(periodType, periodLabel, options = {}) {
  const supervisorProjectFilter = options.activeRole === 'PROJECT_SUPERVISOR'
    ? (options.supervisedProjectIds?.length ? { in: options.supervisedProjectIds } : null)
    : null;
  ...
  const snapshots = await prisma.employeeKpiSnapshot.findMany({
    where: {
      ...
      ...(supervisorProjectFilter ? { user: { operationalProjectId: supervisorProjectFilter } } : {}),
    },
    include: {
  ...
  const assignmentMap = new Map(
    assignments.map((item) => [`${item.userId}::${item.periodType}::${item.periodLabel}`, item]),
  );
  ...
  const enriched = snapshots.map((snapshot) => {
    ...
    const relevantCourses = (coursesByEmployee.get(snapshot.userId) || []).filter((course) =>
      new Date(course.startDate) <= new Date(snapshot.periodEnd) && new Date(course.endDate) >= new Date(snapshot.periodStart),
    );
...
async function getEmployeeSnapshotDetails(userId, periodType, periodLabel, options = {}) {
  ...
  if (options.activeRole === 'PROJECT_SUPERVISOR') {
    const supervisedProjectIds = options.supervisedProjectIds || [];
    if (!supervisedProjectIds.includes(snapshot.user?.operationalProjectId)) {
      const err = new Error('لا تملك صلاحية الاطلاع على هذا السجل');
      err.statusCode = 403;
      throw err;
    }
  }
...
async function getAssignmentRegister(periodType, year, value, options = {}) {
  const { label, start, end } = getPeriodRange(periodType, year, value);

  const employeesWhere = { isActive: true, roles: { has: 'EMPLOYEE' } };
  if (options.activeRole === 'PROJECT_SUPERVISOR') {
    employeesWhere.operationalProjectId = { in: options.supervisedProjectIds || [] };
  }
  ...
  const registerMap = new Map(registers.map((item) => [item.userId, item]));
