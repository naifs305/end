...
async function runReturnedElementReminder(job) {
  ...
  const returnedElements = await prisma.courseClosureTracking.findMany({
    where: {
      status: 'RETURNED',
      decisionAt: { not: null },
      course: { status: { notIn: ['CLOSED', 'ARCHIVED'] } },
...
async function runMedicalInsuranceReminder() {
  ...
  const insuranceEl = await prisma.closureElement.findUnique({
  ...
  const pendingInsurance = await prisma.courseClosureTracking.findMany({
    where: {
      elementId: insuranceEl.id,
      status: { in: ['NOT_STARTED', 'RETURNED'] },
      course: {
        courseType: 'external',
  ...
  for (const tracking of pendingInsurance) {
    ...
    const startStr = new Date(course.startDate).toLocaleDateString('ar-SA-u-ca-gregory', {
    ...
    await createNotification(course.primaryEmployeeId, 'MEDICAL_INSURANCE_REMINDER', title, message, meta);
    notificationsSent++;

    // إشعار مشرفي المشروع
    for (const sup of course.operationalProject?.supervisors || []) {
      await createNotification(sup.userId, 'MEDICAL_INSURANCE_REMINDER', title, message, meta);
      notificationsSent++;
    }

    // إشعار المديرين
    for (const mgr of managers) {
      await createNotification(mgr.id, 'MEDICAL_INSURANCE_REMINDER', title, message, meta);
      notificationsSent++;
    }
  }
