...
async function sendTestEmail({ to, fullName }) {
  const body = `
    <p style="font-size:16px;color:#14221D;">مرحباً <strong>${fullName || ''}</strong>،</p>
    <p style="color:#4B5952;line-height:1.7;">هذه رسالة اختبار من منصة إقفال الدورات التدريبية للتأكد من إعدادات البريد الإلكتروني وعرض شعار الجامعة بشكل صحيح.</p>
