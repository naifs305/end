/**
 * نظام البريد الإلكتروني — Postmark
...
const FROM  = `"منصة إقفال الدورات" <${process.env.POSTMARK_FROM_EMAIL || 'noreply@nauss-closure.sa'}>`;
...
async function send({ to, subject, html, tag }) {
  ...
  try {
    return await client.sendEmail({ From: FROM, To: to, Subject: subject, HtmlBody: html, Tag: tag || 'general', MessageStream: 'outbound' });
  } catch (err) {
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="background:#F0F4F2;padding:20px 32px;text-align:center;border-top:1px solid #E0E7E4;">
          ...
          <p style="margin:4px 0 0;color:#9AA8A1;font-size:10px;">هذا البريد مُرسَل تلقائياً — لا تُرد عليه</p>
        </td></tr>
